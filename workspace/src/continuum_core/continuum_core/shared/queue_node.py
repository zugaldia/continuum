...
class QueueNode(AsyncNode, ABC):
    ...
    @abstractmethod
    def handle_streaming_result(self, streaming_response: ContinuumResponse) -> None:
        """Implemented by each subsystem to support their own request-response types."""
        pass
    ...
    def _queue_request(self, sdk_request: ContinuumRequest) -> None:
        ...
        future = asyncio.run_coroutine_threadsafe(self._executor.execute_request(sdk_request), self._core_loop)
        ...
        with self._request_queue_lock:
            ...
            if len(self._current_requests) < MAX_CONCURRENT_REQUESTS:
                ...
                if self.debug_mode:
                    self.get_logger().info(
                        f"Processing request now. Active: {len(self._current_requests)}/{MAX_CONCURRENT_REQUESTS}"
                    )
            ...
            # Queue is full, discard the request
                ...
                self.get_logger().error(
                    f"Request queue full ({MAX_REQUEST_QUEUE_SIZE}). Discarding request: {sdk_request.session_id}"
                )
